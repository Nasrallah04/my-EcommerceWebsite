...
export function* signInWithEmail({ payload: { email , password } }: EmailSignInStart) {
  try {
    const userCrediential = yield* call(signInAuthUserWithEmailAndPassword, {
      email,
      password,
    });
...
export function* signUp({ payload: { email, password, displayName } }: SignUpStart) {
  try {
    const userCrediential = yield* call(createAuthUserWithEmailAndPassword, {
      email,
      password,
      displayName,
    });
    if (userCrediential) {
      const {user}:any = userCrediential;
...
export function* signInAfterSignUp({
  ...
  yield* call(getSnapShotFromUserAuth, userAuth, additionalInformation );
}
